...
- **Battles run in DMs by default.** Ranked (PvP/CPU fallback), friendly, and tournament battles open private per-player DM panels (`battle["dm_messages"]`, `battle["dm_mode"]`, `battle["summary_channel_id"]`). The final stats embed posts to the origin channel and a personalized recap is DM'd to each player (both once-only via the `stats_sent` gate). If a player's DMs are closed, the battle falls back to the channel with a notice. Keep all battle-message traffic routed through `_battle_targets()` — never touch `message_channel_id`/`dm_messages` directly in new code.
